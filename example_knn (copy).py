...
def feature_vector(train,lines):
    ...
    lower_red1 = np.array([0,150,10])
    upper_red1 = np.array([30,255,255])

    lower_red2 = np.array([160,100,10])
    ...
    for i in range(len(lines)):
        ...
        for c in contours:
            ...
            if len(approx) >= 4:
                ...
                # X_data.append(resized)
                # if h <55:
                #     new_img=curr_image[y:y+2*h,x:x+w]
                #     resized = cv2.resize(new_img, (100,100), interpolation = cv2.INTER_AREA)
                #     X_data.append(resized)
                if h >55:
                    new_img=curr_image[y:y+h,x:x+w]
...
train = np.array([np.array(cv2.imread("./imgs/"+lines[i][0]+".png")) for i in range(len(lines))])
...
with open('./imgs/test.txt', 'rb') as f1:
    reader1 = csv.reader(f1)
    lines1 = list(reader1)


# abc = cv2.imread("./imgs/"+lines1[0][0]+".png")

# print abc.shape

# for i in range(len(lines1)):
# 	a= cv2.imread("./imgs/"+lines1[i][0]+".png")
# 	print a.shape


test = np.array([np.array(cv2.imread("./imgs/"+lines1[i][0]+".png")) for i in range(len(lines1))])
